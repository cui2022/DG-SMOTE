...
def select(individuals, k=SIZE, tournsize=3, fit_attr="fitness"):
    chosen = []
    for i in range(k):
        chose = []
        while len(chose) == 0:
...
file = 'pomeroy-2002-v1'
NGEN = 100
CXPB = 0.8
MUTPB = 0.2
LIMIT = 10
data_name = '.\\data' + file + '.arff'
...
def main(i):
    ...
    fitnesses = []
    for q in range(NUMS):
        toolbox.register("evaluate1", eval, data=less_data, num=q)
    ...
    for g in range(NGEN):
        ...
        fitnesses = []
        for q in range(NUMS):
            toolbox.register("evaluate2", eval, data=less_data, num=q)
...
if __name__ == "__main__":
    ...
    for i in range(30):
        b_time = time.time()
        resultall, resultall_f, result, result_f = main(i)
        n_time = time.time()
